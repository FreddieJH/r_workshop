...
sizes_byspp <-
  jervis %>%
  summarise(
    mean_size = weighted.mean(size_class, n_500m2),
...
nspp_bysurv_bysite %>%
  ...
  ggplot() +
  aes(x = site_code, y = n, col = as.factor(method)) + # why do we need as.factor() here?
  geom_boxplot()
...
nspp_bysurv_bysite %>%
  ...
  ggplot() +
  aes(x = site_code, y = n, col = as.factor(method)) +
  geom_boxplot()
...
nspp_bysurv_bysite %>%
  filter(site_code %in% selected_sites) %>%
  summarise(
    mean_diversity = mean(n, na.rm = TRUE),
...
```{r}
nspp_bysite %>%
  ggplot(aes(x = n, col = as.factor(method))) +
  geom_density()
```
...
nspp_bysurv_bysite %>%
  ...
  summarise(mean_diversity = mean(n, na.rm = TRUE), .by = c(year, method)) %>%
  ggplot() +
