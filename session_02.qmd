...
1.  Download 'rls_example_canada.csv' - [Click here to download](https://raw.githubusercontent.com/FreddieJH/r_workshop/main/rls_example_canada.csv)
...
- Say I have three functions; `import()`, `clean()`, and `visualise()` and I want to apply them to some dataset `x`.
...
# it's hard to read
output <- summarise(filter(read_csv("rls_example_canada.csv"), depth > 10), mean_temp = mean(biomass, na.rm = TRUE))
```
...
output <-
  read_csv("rls_example_canada.csv") %>% 
  filter(depth > 10) %>%
